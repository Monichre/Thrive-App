...
export default class OnBoardForm extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      email: '',
      password: '',
      success_track_template: '',
      successTrackChosen: false,
      successTrackSubmitted: false,
  ...
  componentWillMount() {
    const existing_state = JSON.parse(localStorage.getItem('state'))

    if (existing_state && existing_state.successTrackSubmitted) {
      console.log(existing_state.successTrackSubmitted)
      this.state = existing_state
    }
    console.log(this.state)
  }
  componentDidMount() {
    // console.log(this.defaultProps)
    // let checkbxsCross = Array.prototype.slice.call( document.querySelectorAll( '.ac-cross input[type="checkbox"]' ) )
    // let radiobxsFill = Array.prototype.slice.call( document.querySelectorAll( '.ac-fill input[type="radio"]' ) )
    // console.log(radiobxsFill)
    // let checkbxsCheckmark = Array.prototype.slice.call( document.querySelectorAll( '.ac-checkmark input[type="checkbox"]' ) )
    // let radiobxsCircle = Array.prototype.slice.call( document.querySelectorAll( '.ac-circle input[type="radio"]' ) )
    // let checkbxsBoxfill = Array.prototype.slice.call( document.querySelectorAll( '.ac-boxfill input[type="checkbox"]' ) )
    // let radiobxsSwirl = Array.prototype.slice.call( document.querySelectorAll( '.ac-swirl input[type="radio"]' ) )
    // let checkbxsDiagonal = Array.prototype.slice.call( document.querySelectorAll( '.ac-diagonal input[type="checkbox"]' ) )
    // let checkbxsList = Array.prototype.slice.call( document.querySelectorAll( '.ac-list input[type="checkbox"]' ) )


    // checkbxsCross.forEach(( el, i ) => this.controlCheckbox( el, 'cross' ))
    // // radiobxsFill.forEach(( el, i ) => this.controlRadiobox( el, 'fill' ))
    // checkbxsCheckmark.forEach(( el, i ) => this.controlCheckbox( el, 'checkmark' ))
    // radiobxsCircle.forEach(( el, i ) => this.controlRadiobox( el, 'circle' ))
    // checkbxsBoxfill.forEach(( el, i ) => this.controlCheckbox( el, 'boxfill' ))
    // radiobxsSwirl.forEach(( el, i ) => this.controlRadiobox( el, 'swirl' ))
    // checkbxsDiagonal.forEach(( el, i ) => this.controlCheckbox( el, 'diagonal' ))
    // checkbxsList.forEach(( el ) => this.controlCheckbox( el, 'list', { viewBox : '0 0 300 100', preserveAspectRatio : 'none' } ))
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      this.state.locations_data.push(current_user_position)
    ...
    window.addEventListener('load', function () {
      ...
      const existing_state = localStorage.getItem('state') ? JSON.parse(localStorage.getItem('state')) : false
      if (existing_state) {
        console.log("I should be fucking deleting this goddamn class")
        document.querySelector('#intro').classList.remove("fs-current")
  ...
  componentDidUpdate() {
    console.log(this.state)
    if (this.state.successTrackSubmitted) {
  ...
  handleChosenTrack(e) {
    ...
    this.setState({
      success_track_template: e.target.value,
      successTrackChosen: true
    })

  }
  handleSuccessTrackSubmit(success_track_data) {

    console.log(success_track_data)
    this.setState({
      successTrackSubmitted: true
    })
  ...
  handleIntentOfGoal(e) {
    ...
    this.setState({
      general_assessment: {
        meta_goal_intent: e.target.value
      }
    })
  }
  handleSalaryInput(e) {
  ...
  // 	return svg;
  // }
  // handleFillIn(el, type) {
  //   this.controlRadiobox()
  // }
  // controlRadiobox(el, type ) {
  ...
  //   const element = el
  //   console.log(element)
  //   element.classList.add('filled-in')
  //   const svg = this.createSVGEl()

  //   console.log(element.parentNode)
  //   element.parentNode.appendChild( svg )

  //   this.draw(element, 'fill')
  //   // this.resetRadio( element )
  ...
  // 	switch( type ) {
  // 		case 'cross': pathDef = _this.defaultProps.pathDefs.cross; animDef = _this.defaultProps.animDefs.cross; break;
  // 		case 'fill': pathDef = _this.defaultProps.pathDefs.fill; animDef = _this.defaultProps.animDefs.fill; break;
  // 		case 'checkmark': pathDef = _this.defaultProps.pathDefs.checkmark; _this.defaultProps.animDef = _this.defaultProps.animDefs.checkmark; break;
  // 		case 'circle': pathDef = _this.defaultProps.pathDefs.circle; animDef = _this.defaultProps.animDefs.circle; break;
  // 		case 'boxfill': pathDef = _this.defaultProps.pathDefs.boxfill; animDef = _this.defaultProps.animDefs.boxfill; break;
  // 		case 'swirl': pathDef = _this.defaultProps.pathDefs.swirl; animDef = _this.defaultProps.animDefs.swirl; break;
  // 		case 'diagonal': pathDef = _this.defaultProps.pathDefs.diagonal; _this.defaultProps.animDef = _this.defaultProps.animDefs.diagonal; break;
  // 		case 'list': pathDef = _this.defaultProps.pathDefs.list; animDef = _this.defaultProps.animDefs.list; break;
  // 	};
  ...
  render() {
    ...
    if (this.state.successTrackChosen && !this.state.successTrackSubmitted) {

      return Constants.renderProperTrack(this.state.success_track_template, this.handleSuccessTrackSubmit.bind(this))
        ...
        <div className="fs-form-wrap" id="fs-form-wrap">
          <form id="myform" className="fs-form fs-form-full" autocomplete="off" onSubmit={this.handleSubmit.bind(this)}>
            <ol className="fs-fields">
              <li id="intro" className={this.state.successTrackSubmitted ? ' ' : 'fs-current'}>
              ...
              <li>
                ...
                <input value={this.state.name} onChange={this.handleName.bind(this)} className="fs-anim-lower" id="name" name="name" type="text" placeholder="Dean Moriarty" required autoFocus />
              </li>
              <li>
              ...
              <li data-input-trigger>
                ...
                <div className=" fs-anim-lower fs-radio-group fs-radio-custom clearfix">
              ...
              <li className={this.state.successTrackSubmitted ? 'fs-current' : ''}>
                <label className="fs-field-label fs-anim-upper" for="goal" data-info="Aim high dear friend..">Describe your goal</label>
                <textarea value={this.state.goal_description} onChange={this.handleGoalDescription.bind(this)} className="fs-anim-lower" id="goal" name="goal" placeholder="Describe your goal here"></textarea>
              </li>
              <li>
